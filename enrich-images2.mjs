...
function getImageViaWayback(slug) {
  try {
    // Get latest Wayback snapshot URL
    const gsmaUrl = `gsmarena.com/${slug}`;
    const cdxResp = execSync(
      `curl -sL --max-time 10 "http://archive.org/wayback/available?url=${gsmaUrl}"`,
      { encoding: 'utf8' }
    );
