...
// bigpic filename → hi-res: /vv/pics/{brand}/{filename}-1.jpg
function bigpicToHiRes(bigpicFilename) {
  // e.g. "samsung-galaxy-s25-ultra-sm-s938" → brand="samsung"
  const brand = bigpicFilename.split('-')[0];
  return `https://fdn2.gsmarena.com/vv/pics/${brand}/${bigpicFilename}-1.jpg`;
}

function urlExists(url) {
  try {
    const out = execSync(`curl -sI --max-time 8 "${url}"`, { encoding: 'utf8' });
    return out.includes('HTTP/2 200') || out.includes('HTTP/1.1 200');
  } catch { return false; }
}

function getImageViaWayback(slug) {
  try {
    const cdxResp = execSync(
      `curl -sL --max-time 10 "http://archive.org/wayback/available?url=gsmarena.com/${slug}"`,
      { encoding: 'utf8' }
    );
    const cdx = JSON.parse(cdxResp);
    const snapUrl = cdx?.archived_snapshots?.closest?.url;
    if (!snapUrl) return null;
    const html = execSync(`curl -sL --max-time 15 "${snapUrl}"`, { encoding: 'utf8', stdio: ['pipe','pipe','ignore'] });
    const match = html.match(/https:\/\/fdn2\.gsmarena\.com\/vv\/bigpic\/([^"]+)\.jpg/);
    if (!match) return null;
    // Convert bigpic → hi-res
    const hqUrl = bigpicToHiRes(match[1]);
    return urlExists(hqUrl) ? hqUrl : match[0]; // fallback to bigpic if hi-res 404s
  } catch { return null; }
}

function sleep(ms) { return new Promise(r => setTimeout(r, ms)); }

// Step 1: upgrade existing low-res images to hi-res
console.log('=== Step 1: Upgrading existing images to hi-res ===\n');
const existing = await sql`SELECT id, brand, model, image_url FROM products WHERE image_url LIKE '%fdn2.gsmarena.com/vv/bigpic%'`;
for (const p of existing) {
  const filename = p.image_url.replace('https://fdn2.gsmarena.com/vv/bigpic/', '').replace('.jpg', '');
  const hqUrl = bigpicToHiRes(filename);
  if (urlExists(hqUrl)) {
    await sql`UPDATE products SET image_url = ${hqUrl} WHERE id = ${p.id}`;
    console.log(`✅ HQ upgrade: ${p.brand} ${p.model}`);
  } else {
    console.log(`⚠️  No HQ found, keeping bigpic: ${p.brand} ${p.model}`);
  }
  await sleep(300);
}

// Step 2: enrich remaining missing images (all products without image)
console.log('\n=== Step 2: Enriching missing images ===\n');
const missing = await sql`SELECT id, brand, model FROM products WHERE image_url IS NULL`;
...
for (const p of missing) {
  ...
  if (!slug) {
    console.log(`SKIP [${p.brand} ${p.model}] — TV/no slug`);
    skipped++;
    continue;
  }
  console.log(`Fetching ${p.brand} ${p.model}...`);
  const imgUrl = getImageViaWayback(slug);
  if (imgUrl) {
    await sql`UPDATE products SET image_url = ${imgUrl} WHERE id = ${p.id}`;
    const isHQ = imgUrl.includes('/vv/pics/');
    console.log(`  ${isHQ ? '✅ HQ' : '⚠️  bigpic'} ${imgUrl}`);
    found++;
  } else {
...
console.log(`\n=== DONE ===`);
console.log(`HQ upgrades: ${existing.length} | New images: found=${found} skipped=${skipped} failed=${failed}`);
const total = await sql`SELECT COUNT(*) as total, COUNT(image_url) as with_images FROM products`;
console.log(`DB: ${total[0].with_images}/${total[0].total} products have images`);
